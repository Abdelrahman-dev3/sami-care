                    ...
                    function deleteItem(id, event) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                createNotify({ title: 'خطأ', desc: '<?php echo e(__('messagess.failed_to_remove_item')); ?>' });
                            }
                        })
                        .catch(err => console.log(err));
                    }
  ...
  <form action="<?php echo e(route('payment-chanal')); ?>" method="POST">
    ...
    <div class="page-wrap">
            ...
            <div class="col-lg-8">
                ...
                <div class="panel">
                    ...
                    <div class="method d-flex" style="gap: 20px;flex-direction: column;" data-method="cod" tabindex="0">
                        ...
                        <div class="flex-fill muted"> <?php echo e(__('messagess.wallet_balance_requirement', ['percent' => $codDepositPercentText])); ?> </div>
                    </div>
                </div>
            </div>
            <?php if(request()->has('ids')): ?>
     ...
     <script>
        ...
        const codDepositPercent = Number(<?php echo json_encode($codDepositPercent, 15, 512) ?>);
        const codDepositPercentText = <?php echo json_encode($codDepositPercentText, 15, 512) ?>;
        const codWalletWarning = <?php echo json_encode(__('messagess.wallet_balance_requirement_notify', ['percent' => $codDepositPercentText]), 512) ?>;
        ...
        document.querySelector('form[action="<?php echo e(route('payment-chanal')); ?>"]')?.addEventListener('submit', function (e) {
        ...
        document.querySelector('#applyCoupon').addEventListener('click', function() {
            ...
            if (!couponCode) {
                createNotify({ title: 'خطأ', desc: '<?php echo e(__('messagess.enter_coupon_code')); ?>' });
                return;
            }
            ...
            .then(data => {
                if (data.valid) {
                    createNotify({ title: 'نجاح', desc: "<?php echo e(__('messagess.coupon_applied')); ?>: " + couponCode });
                    let discount = 0;
                    ...
                    createNotify({ title: 'خطأ', desc: "<?php echo e(__('messagess.invalid_coupon')); ?>" });
                }
            })
            .catch(() => { createNotify({ title: 'خطأ', desc: "<?php echo e(__('messagess.error_occurred')); ?>" }); });
        });
    </script><?php /**PATH C:\Users\VIP\Desktop\sami-care\sami-care\resources\views/components/frontend/payment.blade.php ENDPATH**/ ?>