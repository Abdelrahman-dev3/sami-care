...
<div class="container py-5">
  <div class="row g-4">
        ...
        <table class="table align-middle">
          <thead>
            <tr style="background-color: red;">
                ...
                <th style="padding:16px 20px;font-weight:bold;">تقييم التجربة</th>
            </tr> 
          </thead>
          <tbody>
                ...
                <tr>
                  ...
                  <td>
                      ...
                      <button type="button" class="btn btn-sm" 
                              style="background: linear-gradient(135deg, #BF9456, #d4a96a); color: #fff; border: none;"
                              onclick="openBookingReviewModal({{ $booking->id }})">
                          <i class="fas fa-comment-dots"></i> قيّم تجربتك
                      </button>
                      @endif
                  </td>
                </tr>
    ...
    <div class="modal fade" style="z-index: 999999999999999999999;" id="bookingReviewModal" tabindex="-1" aria-labelledby="bookingReviewModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header" style="background: linear-gradient(135deg, #1a1a2e, #16213e); border: none;">
            <h5 class="modal-title" id="bookingReviewModalLabel" style="color: #BF9456; font-weight: 700;">
                <i class="fas fa-star" style="margin-left: 6px;"></i> قيّم تجربتك
            </h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body" id="bookingReviewModalBody">
            <div class="text-center mb-4">
                <p style="color: #666; font-size: 0.95rem;">شاركنا رأيك عن تجربتك في سامي كير</p>
            </div>
            <div class="mb-3 text-center">
                <label class="d-block mb-2" style="font-weight: 600;">التقييم:</label>
                <div class="star-rating" style="justify-content: center;" id="bookingStarRating">
                    <input type="radio" name="booking_rating" value="5" id="bstar5" class="star-input" checked>
                    <label for="bstar5" class="star-label"><i class="fa-solid fa-star"></i></label>
                    <input type="radio" name="booking_rating" value="4" id="bstar4" class="star-input">
                    <label for="bstar4" class="star-label"><i class="fa-solid fa-star"></i></label>
                    <input type="radio" name="booking_rating" value="3" id="bstar3" class="star-input">
                    <label for="bstar3" class="star-label"><i class="fa-solid fa-star"></i></label>
                    <input type="radio" name="booking_rating" value="2" id="bstar2" class="star-input">
                    <label for="bstar2" class="star-label"><i class="fa-solid fa-star"></i></label>
                    <input type="radio" name="booking_rating" value="1" id="bstar1" class="star-input">
                    <label for="bstar1" class="star-label"><i class="fa-solid fa-star"></i></label>
                </div>
            </div>
            <div class="mb-3">
                <label class="d-block mb-2" style="font-weight: 600;">رأيك (اختياري):</label>
                <textarea class="form-control" id="booking_review_text" rows="3" 
                          placeholder="اكتب رأيك عن التجربة..." style="border-radius: 10px;"></textarea>
            </div>
            <input type="hidden" id="booking_review_booking_id">
            <button class="btn w-100" onclick="submitBookingReview()" 
                    style="background: linear-gradient(135deg, #BF9456, #d4a96a); color: #fff; border: none; padding: 12px; border-radius: 10px; font-weight: 600;">
                <i class="fas fa-paper-plane" style="margin-left: 6px;"></i> إرسال التقييم
            </button>
          </div>
        </div>
      </div>
...
<script>
    ...
    function openBookingReviewModal(bookingId) {
        document.getElementById('booking_review_booking_id').value = bookingId;
        document.getElementById('booking_review_text').value = '';
        document.querySelector('input[name="booking_rating"][value="5"]').checked = true;

        // Check if user already reviewed this booking
        fetch(`/booking-review/${bookingId}`)
            .then(response => response.json())
            .then(data => {
                if (data.has_review && data.data) {
                    document.getElementById('booking_review_text').value = data.data.review_text || '';
                    const ratingInput = document.querySelector(`input[name="booking_rating"][value="${data.data.rating}"]`);
                    if (ratingInput) ratingInput.checked = true;
                }
            })
            .catch(err => console.error(err));

        var modal = new bootstrap.Modal(document.getElementById('bookingReviewModal'));
        modal.show();
    }

    function submitBookingReview() {
        const bookingId = document.getElementById('booking_review_booking_id').value;
        const rating = document.querySelector('input[name="booking_rating"]:checked').value;
        const reviewText = document.getElementById('booking_review_text').value;
        ...
        .then(data => {
            if (data.status) {
                createNotify({ title: '', desc: data.message });
                var modal = bootstrap.Modal.getInstance(document.getElementById('bookingReviewModal'));
                modal.hide();
            } else {
                createNotify({ title: '', desc: data.message || 'حدث خطأ' });
            }
        })
        .catch(error => {
            console.error('Error:', error);
