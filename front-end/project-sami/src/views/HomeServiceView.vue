...
usePageStyles(pageCss, 'home-service')

const page = ref({ title: 'الخدمة المنزلية', subtitle: '', sections: [] })
...
async function loadPage() {
  ...
  error.value = ''
  try {
    const result = await authFetch('/home-service-page')
    if (request === pageRequest) page.value = result.data
  } catch {
    if (request !== pageRequest) return
    error.value = 'تعذر تحميل تفاصيل الخدمة المنزلية. حاول مرة أخرى.'
  } finally {
    ...
    <div class="wrap">
      ...
      <div class="legal-content" :aria-busy="loading">
        ...
        <template v-else>
          ...
          <p v-if="!page.sections.length">سيتم إضافة تفاصيل الخدمة المنزلية قريبًا.</p>
        </template>
      </div>
    </div>
  </div>
