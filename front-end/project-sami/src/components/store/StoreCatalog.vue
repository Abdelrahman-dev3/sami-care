...
import { STORE_PROMOS } from '@/data/store'
...
const catList = computed(() => {
  ...
  return categories.value.slice(1).concat(categories.value[0])
})

/* عدد المنتجات بعد التصفية قبل القص — للعدّاد وزر "عرض الكل" */
const filteredCount = computed(() => {
  const l = products.value.filter(p => state.cat === 'all' || p.cat === state.cat)
  return l.length
})

const bestProducts = computed(() => products.value.filter(p => p.best))
...
const catShape = id => {
  ...
  return shapeParts(c ? c.shape : 'kit', 52)
}
